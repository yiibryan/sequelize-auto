...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var modelName_forES6 = Inflector.pluralize(tableName);
      modelName_forES6 = modelName_forES6[0].toUpperCase() + modelName_forES6.substring(1);
      ...
      if(!self.options.typescript){
        ...
        if (self.options.es6 === 'es6') {
          ...
          text[table] += spaces + "let " + modelName_forES6 + " = sequelize.define('" + tableName + "', {\n";
        } else {
      ...
      if (self.options.es6 === 'es6') {
        ...
        text[table] += spaces + spaces  + "    //exp."+ modelName_forES6 +".hasOne(DataModel.TableA);\n";
        text[table] += spaces + spaces  + "  }\n";
        text[table] += spaces + spaces  + "},\n";
      }
      ...
      text[table] += ");\n";
      if (self.options.es6 === 'es6') {
        text[table] += spaces + "return " + modelName_forES6 + ";\n";
      }
      text[table] += "};\n";
      _callback(null);
    }, function(){
