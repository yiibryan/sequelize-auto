...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var modelNameForES6 = Inflector.pluralize(tableName);
      modelNameForES6 = modelNameForES6[0].toUpperCase() + modelNameForES6.substring(1);

      if(!self.options.typescript){
        text[table] = "/* jshint indent: " + self.options.indentation + " */\n\n";
        if (self.options.es6 === 'es6') {
          text[table] += "export default (sequelize, DataTypes)=> {\n";
          text[table] += spaces + "let " + modelNameForES6 + " = sequelize.define('" + tableName + "', {\n";
        } else {
        ...
        _.each(fieldAttr, function(attr) {
          ...
          if (attr === "foreignKey") {
            ...
            if (self.tables[table][field].type.toLowerCase() === 'bit(1)') {
              valText = defaultVal === "b'1'" ? 1 : 0;
            }
            // mssql bit fix
            else if (self.sequelize.options.dialect === "mssql" && self.tables[table][field].type.toLowerCase() === "bit") {
              valText = defaultVal === "((1))" ? 1 : 0;
            }

            if (_.isString(defaultVal)) {
              var fieldType = self.tables[table][field].type.toLowerCase();
              if (_.endsWith(defaultVal, '()')) {
                valText = "sequelize.fn('" + defaultVal.replace(/\(\)$/, '') + "')"
              }
                 ...
                 if (_.includes(['current_timestamp', 'current_date', 'current_time', 'localtime', 'localtimestamp'], defaultVal.toLowerCase())) {
                  valText = "sequelize.literal('" + defaultVal + "')"
                } else {
                  valText = quoteWrapper + valText + quoteWrapper
                }
              } else {
                valText = quoteWrapper + valText + quoteWrapper
              }
            ...
            if(defaultVal === null || defaultVal === undefined) {
              ...
              valText = _.isString(valText) && !valText.match(/^sequelize\.[^(]+\(.*\)$/) ? SqlString.escape(_.trim(valText, ['"',"'"]), null, self.options.dialect) : valText;

              // don't prepend N for MSSQL when building models...
              valText = _.trimStart(valText, 'N')
              text[table] += spaces + spaces + spaces + attr + ": " + valText;
            }
          }
          ...
          } else if (attr === 'comment' && _.trim(self.tables[table][field][attr]).length > 0) {
            text[table] += spaces + spaces + spaces + attr + ": '" + self.tables[table][field][attr].replace(/\n/g, " ") + "'";
          } else {
            ...
            else if (_attr.match(/^(float8|double precision|double|numeric)/)) {
      ...
      if (self.options.es6 === 'es6') {
        ...
        text[table] += spaces + spaces  + "  associate: (DataModel)=>{\n";
        text[table] += spaces + spaces  + "    //param DataModel, Object-Array contains other Models.\n";
        text[table] += spaces + spaces  + "    //TODO:set up the associate to other Models. refer to: https://github.com/sequelize/express-example/blob/master/models/index.js \n";
        text[table] += spaces + spaces  + "    //exp."+ modelNameForES6 +".hasOne(DataModel.TableA);\n";
        text[table] += spaces + spaces  + "  }\n";
        text[table] += spaces + spaces  + "},\n";
      }

      if(self.options.schema) {
        text[table] += spaces + spaces  + "schema: '" + self.options.schema + "',\n";
      }
      ...
      text[table] += ");\n";
      if (self.options.es6 === 'es6') {
        text[table] += spaces + "return " + modelNameForES6 + ";\n";
      }
      text[table] += "};\n";
      _callback(null);
    }, function(){
