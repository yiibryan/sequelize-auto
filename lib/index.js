...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if (self.options.es6 === 'es6') {
        text[table] += "classMethods: {\n";
        text[table] += "  associate: (DataModel)=>{\n";
        text[table] += "    //param DataModel, Object-Array contains other Models.\n";
        text[table] += "    //TODO:set up the associate to other Models. refer to: https://github.com/sequelize/express-example/blob/master/models/index.js \n";
        text[table] += "    //exp."+ modelName_forES6 +".hasOne(DataModel.TableA);\n";
        text[table] += "  }\n";
        text[table] += "},\n";
      }
